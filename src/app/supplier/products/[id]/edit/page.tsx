...
export default function SupplierEditProductPage() {
    ...
    fetch(`/api/supplier/products/${id}`)
      ...
      .then(data => {
        setForm(f => ({
          ...
          quantity: data.stock ? String(data.stock) : "",
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || "Erreur modification produit");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 via-white to-orange-50 py-8 flex items-center justify-center">
      <div className="w-full max-w-2xl mx-auto bg-white/90 rounded-2xl shadow-2xl p-8">
        <h1 className="text-2xl font-bold mb-6 text-center text-amber-600">Modifier le produit</h1>
